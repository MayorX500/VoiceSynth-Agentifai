...
# Logging parameters
RUN_NAME = "GPT_XTTS_PT_PT_AUDIOBOOK"
PROJECT_NAME = "XTTS_trainer_PT_PT_AUDIOBOOK"
...
BATCH_SIZE = 1
GRAD_ACUMM_STEPS = 32  # Adjust to a higher number
...
config_dataset = BaseDatasetConfig(
    ...
    path="../datasets/o_corvo/",  # Path to your dataset directory
...
def main():
    ...
    train_samples, eval_samples = load_tts_samples(
        ...
        eval_split=True,
        eval_split_size=0.2,  # Increase the evaluation split size to 30%
    )


    print(f"Number of training samples: {len(train_samples)}")  # Print the number of training samples
    print(f"Number of evaluation samples: {len(eval_samples)}") # Print the number of evaluation samples

    # Assign unique speaker IDs to each audio file
    for sample in train_samples:
        sample['speaker_id'] = "single_speaker"
